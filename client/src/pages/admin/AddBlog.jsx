...
const AddBlog = () => {
  ...
  const [image, setImaage] = useState(false);
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
      <div className='bg-white w-full max-w-6xl p-4 md:p-10 sm:m-10 shadow rounded'>

        <p>Upload Thumbnail</p>
        <label htmlFor="image">
          <img src={!image ? assets.upload_area : URL.createObjectURL(image)} alt="" className='mt-2 h-16 rounded cursor-pointer'/>
          <input onChange={(e)=> setImaage(e.target.files[0])} type="file" id='image' hidden required />
        </label>
